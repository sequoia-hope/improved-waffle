...
- [ ] Phase 2 (app storage) — spec §7, §9:
  ...
  - [ ] P2-3 source resolution at open: bridge `ListSources` (entries + availability) → cache/adapters → `ProvideSource`; "Import STEP from link" (a `{Step, Git, pack:false}` source + ImportedBody)
  - [ ] P2-4 pack/unpack + pin/update-to-tip UI (a Sources panel; bridge `UpdateSourceEntry`)
  - [ ] P2-5 `document.id` as the storage key (8-char ids become aliases); `GitProvider` generalization (GitLab/Gitea as storage, folders, per-host tokens for storage; `.waffle-index.json` keyed by `document.id`)
