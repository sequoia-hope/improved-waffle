...
- [ ] Phase 2 (app storage) — spec §7, §9:
  ...
  - [ ] P2-5b `GitProvider` generalization (GitLab/Gitea as storage providers, folders, per-host tokens for storage; `.waffle-index.json` keyed by `document.id` for new entries)
