...
- **Slice F-3 — torus DISK patch. ✅ DONE (2026-09-04).** R0032's
  ...
  **Assay: R0032 moves `UNSUPPORTED(curved-profile)` → its next real wall,
  ...
  3 → 2 (R0044 M5 K11, C0063 apex-cone operand remain). **2026-09-04, later
  the same day: the class is EMPTY** — C0063 by the apex-cone operand, R0044
  by M5 K11 inc-1 (R0044 ERROR at the thin-band chart crossing above).
...
|---|---|---|---|
| R0044 | `FaceId(458)`, op `boolean_subtract` (the circle cut on the design result) | a CYLINDER lateral (r = 2327.8), no inner loop, a 5-edge outer loop `[Arc, SurfacePair, SurfacePair, SurfacePair, SurfacePair]` — every `SurfacePair` is this cylinder × a CONE (three distinct cones, half-angles 1.011 / 1.048 / 0.440: the revolve's conical flanks) | **M5 K11**: no yang INPUT vocabulary for a degree-4 surface-pair edge. The re-entry needs the procedural (Option B) cylinder∩cone curve sampled onto the cylinder's (θ, z) chart as a shared boundary chain (twin-identical, conformal to the neighbouring cone's own chain), then the Slice-D CDT; downstream, the chain is a CARRIED input curve for §4.4.2 restoration / §4.5.1 relocation onto the procedural curve. A multi-session capability (roadmap M5), not a slice of this spec. **inc-1 DONE 2026-09-04** (`m5_surface_pair_curve.md` "K11 re-entry"): the Stage-1 wall falls — the pair edges convert and their Newton-certified chains splice; R0044 now STOPs at the NEXT wall, `face 166: holed lateral CDT failed: CDT backend failed to triangulate` — a CONE face of the gear revolve that is a THIN BAND (two rim arcs at radii 3681.15 and 3683.06 about the gear axis, ~2 units apart at scale 4e3, closed by a hyperbola and ONE pair edge) whose rim-arc chords, sampled at the 1e-2 chord bound (sag ≈ 10.6 units ≫ the band width), interleave and CROSS in the cone chart (20 constraint crossings, all arc-chain × arc-chain; the pair edge is a single 1-chord segment and is crossed, not crossing). The thin-band chord-density class (the R0095 "holed lateral CDT failed" sub-wall): Stage 1 must sample a band's rims at a sag below the band's own width for the chart boundary to stay simple. |
