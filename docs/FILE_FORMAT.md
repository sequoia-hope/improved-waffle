...
| `connectors` | MateConnector[] | default `[]`, omitted when empty | `{id, name, instance_path: [UUID, …] (the top-level instance, then members through sub-assemblies — 3d-2), geom_ref? (a face of the PART, §8), frame: {origin, z_axis (default +z), x_axis (default: chosen deterministically)}}`. With `geom_ref`, the frame is derived from the current geometry at evaluation (origin on the planar face, z = outward normal, `frame.x_axis` as the secondary direction when set); without, `frame` is the frame. Coordinates are the part's own. |
