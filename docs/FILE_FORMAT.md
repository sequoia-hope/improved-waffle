...
| Storage envelope — IndexedDB | `app/src/lib/storage/indexeddb.js` | DB `waffle-iron`, store `documents`, records `{id, json: <the .waffle text>, created, modified, link?}` (epoch ms). The `.waffle` JSON travels as an opaque string. `link` (Phase 2, `open-link.js` `DocumentLink`: `{locator, resolved: {commit, at}, contentHash, readOnly: true, name}`) marks a record created from a share link: the app treats it as **read-only** (no autosave, Ctrl+S refused) until "Fork to edit" copies it into an ordinary record with a new `document.id`. A separate DB `waffle-iron-cache` (`git/cache.js`) caches fetched source bytes by `content_hash`. |
