...
**State:** RESOLVED 2026-07-21 — the user reviewed the original
render-floor criterion and DIRECTED the swap to the paper's own
curve-refinement acceptance test, which is now what ships
(`paper_chain_sample_redundant`): a relocated seam sample m between kept
chain neighbours p, q is dropped iff **h < d_p·10², l < d_p·10³,
α < π/18** (arc height, chord length max(|pm|,|mq|), turning angle) —
verbatim the §4.3.4 termination condition
(`refs/text/yang2025_hybrid_boolean.txt:586-592`), i.e. the resulting
polyline is one the paper's own refinement loop would terminate at. The
paper pins d_p = 1e-7 (`:744-745`), which equals this port's `TAU_MODEL`
exactly; the sole port convention applied is the uniform
scale-relativization d_p = TAU_MODEL·(1+scale). Eligibility fencing
unchanged (`moved`, non-mint, single-pair, all-triangles-in-region). The
earlier render-floor criterion (flagged here as N50-adjacent) is RETIRED
from the drop path; the render floor survives only in the fail-closed
postcondition (reject-only, never a merge). Measured under the paper
criterion: R0061 gate-ON SUPPORTED_CORRECT (all checks); 4 predicate
red/green pins (each bound individually violated → keep); full-assay
ledger in the inc-4c-2b commit. History preserved below for the audit
trail; the original PENDING framing is superseded.
...
§4.3/§4.3.4: point removal on intersection polylines is core paper
machinery ("remove a point if it is too close to another point on the
same loop", `:535-536`; the reversal handler "repetitively remove[s] the
next point and reconnect[s] the curve", `:746-748`), and the drop's
criterion is now the paper's OWN refinement acceptance test with the
paper's OWN d_p. The one thing the paper never faces is the misorder
itself: its polylines are built by parameter-domain midpoint subdivision
(`:579-583`) and are ordered by construction — the disorder this pass
sorts out is an artifact of the port's N2 relocation-only Stage 4, and
...
**State:** RESOLVED (2026-08-26, the §4.4.2 flip commit; P5 convention —
mutation-tested red/green oracles, full assay 0 WRONG, no CORRECT
regressions). The restoration is ALWAYS-ON (`YANG_434_OUT=0|off` is the
dev off-knob; `census` stays read-only). Both flip blockers were fixed
structurally the same day (spec inc-4/inc-5): R0054 by kernel-v2
conformal grid-aligned arc sampling (global azimuth grid + conforming
vertex inserts, band-free, always-on); F0085 by aligning
`validate_planar_face`'s loop-vertex planarity check with its DOCUMENTED
debug-only tier (the F1 TAU_EVAL gate is the production contract; the
strict 1e-12 tier fired in release on a chained-re-entry precision
ratchet at a triple-plane corner). A third latent unmasked by the flip
(kernel-v2 barrel-arm hole windowing — a hole left at a ±span image of
its in-ring position is outside the outer polygon and its corridor is
silently FILLED; spec inc-7) was fixed always-on the same day. Flip
corpus: **271C/0W/36E/1EE/0T** (+R0020, +R0095, +C0105 — the
hole-window latent was C0105's selfx root; seven explained detail
shifts, zero CORRECT regressions). RESIDUAL (no standing divergence measured): only `Circle`
carried curves are restored — the inc-0/inc-1 censuses found zero
non-circle carried-edge customers corpus-wide; the restore pass counts
every decline (`[s434-restore]` stats). A non-circle customer, if one
ever appears, enters the ledger as its own new entry. NEW capability gap
recorded (not a deviation): the M8 n-ary mixed-arc vocabulary declines
seam-split (4-arc) strip laterals — zero corpus customers, named by the
quarantined `nary_tessellated_group_stage0_meshes` fixture (spec inc-6).
...
|---|---|---|---|---|
| `f32` | `weld_f32_render_twins` (N50) | f32 bit-key rounding | ~~Stage-0 overlay near-coincident event columns~~ **(N56: the ONLY real hack — render-precision, not geometric; regresses C0036)** | **STAYS RETIRED (N56):** redundant — its cases recover via §4.3 dedup below |
| `coincident` | `weld_coincident_relocated` (N47) | `TAU_MODEL·(1+scale)` | ~~same~~ **(N56: mis-bucketed — it IS Yang §4.3 curve-point dedup)** | **DONE (N56):** genuine §4.3 (both verts on curve), reinstated always-on; 0-conversion (near-tangency infra) |
...
| `subres` | `collapse_subresolution_intersection_segments` (KV15b) | absolute `TAU_MODEL` segment length | ~~Stage-0 overlay sub-resolution crossing mint~~ **(N56: mis-bucketed — it IS Yang §4.3 curve-point dedup)** | **DONE (N56):** retighten to scale-relative `TAU_MODEL·(1+scale)`, reinstated always-on; recovers R0076/R0088/F0078/F0079/F0084 |
