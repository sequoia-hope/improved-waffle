...
> **2026-09-12 (night, later) refresh.** Canonical corpus **285C / 0W / 20E / 4EE / 0T + 3 UNSUPPORTED(coplanar-boolean)** (release, 8 jobs, 600 s; wall 848.7 s at host load ≈ 10; F0085 369.2 s, R0044 350.5 s) — category-identical. The I6.6 pleat cancellation at the `boolean()` compaction is now the f64-AREA form (spec `yang_146_collapsed_wedge_dedup.md` §7.1: a pair cancels iff its triple's height above the longest edge is within the rounding band — the bunched pleat AND the needle): R0019 (a B strip diagonal between two rim-junction mints lying in A's cap plane, sliced by A's cap triangles' slightly different exact planes into 3.9e-4-long, 1.3e-18-high opposite-winding slivers) advances from the I6 backstop to kernel-v2's ring-reject (FaceId 651), R0085 op 3 from the backstop to Stage-6 non-2-manifold. `NonManifoldInput` family EMPTY.
>
...
> corpus-run, nor were the 24 app / file-format commits after it; R0017 and R0059 (both since
> converted) and R0025 are ULP-parse-sensitive latents (bisected + proven by the
